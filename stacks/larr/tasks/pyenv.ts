...
export async function run(ctx: TaskContext): Promise<void> {
  ...
  // 3. Install Python 3.13.13
  log.info("Installing Python 3.13.13 via pyenv");
  // apt installs pyenv to /usr/bin/pyenv, versions go to ~/.pyenv/versions/
  await runOrFail(ctx, ["pyenv", "install", "-s", "3.13.13"]); // -s skips if exists

  // 4. Set as global default
  log.info("Setting Python 3.13.13 as global default");
  await runOrFail(ctx, ["pyenv", "global", "3.13.13"]);

  // 5. Rehash to create/update shims (needed for python command to work)
  await runOrFail(ctx, ["pyenv", "rehash"]);

  log.success("pyenv with Python 3.13.13 installed");
}

export async function verify(ctx: TaskContext): Promise<void> {
  await v.assertCommand("pyenv", "--version");
  await v.assertDir(`${ctx.home}/.pyenv/versions/3.13.13`);
